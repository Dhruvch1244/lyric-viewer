...
/**
 * App version, filled from main once get-prefs resolves. Declared here
 * (rather than in onboarding-cards.js, which reads and shows it) because
 * this file's own getPrefs().then() callback writes it — and that callback
 * can fire before onboarding-cards.js's own <script> tag has even started
 * loading, since a rejected/resolved promise's microtask runs at the end of
 * THIS script's execution, before the parser moves on to the next one.
 */
let appVersion = null;
...
/*
  ...
  anything is playing. That is the opposite of the capture nudge (see
  onboarding-cards.js), which waits 20s into a song precisely because it is
  interrupting something that is already going well.

  This cluster (welcome + what's-new) stays here rather than moving to
  onboarding-cards.js with the rest of the notification cards: it's reached
  from window.player.getPrefs().then() below, and a promise callback attached
  to an already-fast/rejected invoke can run its microtask before the next
  <script> tag has even started loading — proven by an actual ReferenceError
  caught while extracting this file the first time. Everything else in
  onboarding-cards.js is reached only via a real event registration
  (onTick/onUpdateState/onLocalcliOffer) or a DOM listener, which are
  genuinely deferred past every script tag finishing, so they extracted clean.
*/
