...
exports.toDevanagari = toDevanagari;
...
async function convertBatch(lines, convert) {
    const parsed = await convert({
        system: SYSTEM_PROMPT,
        user: 'Transliterate these lyric lines to Devanagari. Return one output line per input line.\n\n' +
            JSON.stringify({ lines }, null, 2),
        schema: SCHEMA,
    });
    const out = parsed.lines;
    if (!Array.isArray(out) || out.length !== lines.length) {
        throw new Error(`Line count mismatch: sent ${lines.length}, received ${Array.isArray(out) ? out.length : 0}.`);
    }
    return out.map((line) => String(line));
}
/** Transliterate a full cue list into Devanagari, preserving timing. */
async function toDevanagari(cues, convert) {
