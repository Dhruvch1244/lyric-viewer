 ...
 */
export function scoreCandidate(candidate: LrclibCandidate, target: TrackQuery): number {
  ...
  if (target.durationMs > 0 && (candidate.duration || 0) > 0) {
    const deltaSec = Math.abs((candidate.duration as number) - target.durationMs / 1000);
...
export function isDevanagariCues(cues: LyricCue[]): boolean {
  const sample = cues
    .slice(0, 12)
    .map((c) => c.text)
    .join(' ');
...
export function gurmukhiRatio(text: string): number {
  if (!text) return 0;
  const letters = text.match(/\p{L}/gu) || [];
  if (letters.length === 0) return 0;
 ...
 */
export function detectIndic(cues: LyricCue[]): IndicDetection {
 ...
 */
export async function fetchSyncedLyrics(track: TrackQuery): Promise<SyncedLyricsResult | null> {
  ...
  try {
    ...
    throw new Error(`Lyric lookup failed: ${(err as Error).message}`);
  }
  ...
  let bestScore = 0;
  let devaBest: LrclibCandidate | null = null;
  let devaBestScore = 0;
  ...
  const cues = parseLrc(best.syncedLyrics as string);
  if (cues.length === 0) return null;

  // If the primary match is itself Devanagari, it doubles as the native track.
  let cuesDevanagari: LyricCue[] | null = null;
  if (isDevanagariCues(cues)) {
    cuesDevanagari = cues;
  } else if (devaBest && devaBestScore >= 0.5) {
    const parsed = parseLrc(devaBest.syncedLyrics as string);
